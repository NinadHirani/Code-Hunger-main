...
export const sampleProblems: DetailedProblem[] = [
  {
    title: "Two Sum",
    slug: "two-sum",
    difficulty: "Easy",
...
export function getDetailedProblem(slugOrId: string): DetailedProblem | undefined {
  if (!slugOrId) return undefined;
  const normalized = slugOrId.toLowerCase().trim();
  return sampleProblems.find(p => p.slug.toLowerCase() === normalized || (p.id && p.id.toLowerCase() === normalized));
}

export function getAllDetailedProblems(): DetailedProblem[] {
